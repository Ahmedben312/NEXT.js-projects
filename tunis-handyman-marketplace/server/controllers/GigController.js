...
export const searchGigs = async (req, res, next) => {
  try {
    ...
    apiLog(
      `🔍 Search request: "${searchTerm}" | Category: "${category}" | Featured: ${featured}`,
    );
    ...
    if (featured) {
      ...
      gigs = await SearchService.fuzzySearch(searchTerm, category);
      if (limit > 0) {
        gigs = gigs.slice(0, limit);
      }
    } else {
      // Return all gigs if no search params
      gigs = await prisma.gig.findMany({
        include: {
          createdBy: true,
          reviews: true,
